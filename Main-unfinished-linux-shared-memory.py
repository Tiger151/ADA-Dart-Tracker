...
alive = True #global var to switch sub processes to calculations
...
def SideCam():
	...
	while cap.isOpened() and alive: #while camera or video is open
...
if __name__ == '__main__':
	...
	print("==================CAMERAS START==================")
	sideT = multiprocessing.Process(target = SideCam)
	bottomT = multiprocessing.Process(target = BottomCam)
	sideT.start()
	bottomT.start()
	...
	while True:		
		...
		if deltaTime >= 3 and deltaTime < 5:
			alive = False
		if deltaTime > 5: #5 second auto timeout
			sideT.terminate()
			bottomT.terminate()
