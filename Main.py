...
def SideCam():
    ...
    while cap.isOpened(): #while camera or video is open
        ...
        if deltaTime > 3: #3 second auto timeout
            break
    ...
    print("Velocity = 1m per", velocity, "sec")
    print("theta", theta)
...
#Multi threading to run 2 camera instances at same time
Thread(target = SideCam).start()
Thread(target = BottomCam).start()